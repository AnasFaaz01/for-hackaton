...
import { Hand, RotateCcw, Camera, StopCircle, Pause, ThumbsUp, ThumbsDown, HelpCircle, Droplets, Activity, Fingerprint, Volume2 } from "lucide-react";
import { useHandGesture } from "@/hooks/useHandGesture";
import { HAND_GESTURE_MAP } from "@/types";
import { voiceAlert } from "@/lib/tts";

const GESTURE_GUIDE = [
  { label: "YES", desc: "Thumbs Up", icon: ThumbsUp, color: "bg-teal-50 text-teal-600" },
  { label: "NO", desc: "Thumbs Down", icon: ThumbsDown, color: "bg-red-50 text-red-500" },
  { label: "HELP", desc: "Index & Pinky Extended", icon: HelpCircle, color: "bg-amber-50 text-amber-500" },
  { label: "WATER", desc: "Both Hands Open", icon: Droplets, color: "bg-cyan-50 text-cyan-600" },
  { label: "PAUSE", desc: "Open palm wide 5s to toggle", icon: Pause, color: "bg-slate-100 text-slate-500" },
];
          ...
          <div className="flex items-center justify-between flex-wrap gap-4">
            <div>
              ...
              <p className="mt-1 text-sm text-slate-500">
                Raise your hand and make a gesture. Open palm wide and hold 5s to pause.
              </p>
            </div>
            {cameraOn && (
          ...
          <div className="lg:col-span-2 space-y-4">
            <div className="relative overflow-hidden rounded-xl aspect-[4/3] border border-slate-200 bg-white">
              {loading && (
                <div className="absolute inset-0 flex items-center justify-center bg-white z-20">
                  <div className="text-center">
                    ...
                    <p className="text-slate-700 text-sm font-medium">Loading hand tracking model...</p>
              ...
              {cameraOn && (
                <div className="absolute top-3 left-3 px-2.5 py-1 rounded-lg bg-white/90 border border-slate-200 text-slate-700 text-xs font-medium flex items-center gap-1.5 shadow-sm">
                  <span className={`w-1.5 h-1.5 rounded-full ${isPaused ? "bg-amber-500" : numHands > 0 ? "bg-teal-500" : "bg-slate-300"}`} />
            ...
            {cameraOn && (
              <div className="dashboard-card p-3 flex items-center gap-3">
                <span className={`flex items-center gap-1.5 text-sm ${isPaused ? "text-amber-600" : numHands > 0 ? "text-teal-600" : "text-slate-400"}`}>
                  <span className={`w-1.5 h-1.5 rounded-full ${isPaused ? "bg-amber-500" : numHands > 0 ? "bg-teal-500" : "bg-slate-300"}`} />
            ...
            <AnimatePresence mode="wait">
              {isPaused ? (
                ...
                >
                  ...
                  <div className="text-xs text-slate-400">Show open palm wide to resume</div>
                </motion.div>
                ...
                >
                  ...
                  <p className="text-slate-600 font-medium">Waiting for gesture...</p>
                  <p className="text-slate-400 text-xs mt-1">Make a gesture to see the result</p>
                </motion.div>
              )}
            </AnimatePresence>
